...
export class AccnComponent implements OnInit, OnDestroy {
  ...
  public cfgmd:accn_cfg;
  public cacls:accn_ls;
  public roleselect:number;
  constructor(private sv: adminService, private av: authService, private ss:shareService, private router: RouterModule,private toastr: ToastrService,private ngPopups: NgPopupsService) { 
  ...
  accnget(o:accn_ls,ix:number) { 
    this.cacls = o;
    this.accselect = ix;
    this.sv.accnMod(o).pipe().subscribe(            
      (res) => { this.mdaccn = res; },
  ...
  selWarehouse(){
    this.sv.sellovrole(this.slcwarehouse.value,this.slcwarehouse.valopnfirst).pipe().subscribe( (res) => { 
      // re get role
        this.lsrole = res;  
  ...
  addCfg(){
    this.ngPopups.confirm('Do you confirm to config role ' + this.slcrole.desc +' ?')
    .subscribe(res => {
      if (res) {
        ...
        this.sv.addCfg(this.cfgmd).pipe().subscribe( (res) => { 
  ...
  delCfg(){
    if(this.mdaccn.accncfg.filter(x=>x.site == this.slcwarehouse.value && x.rolecode == this.slcrole.value).length == 0){
      this.toastr.error("<span class='fn-07e'>please selected role config</span>",null,{ enableHtml : true });
    }else {
      this.ngPopups.confirm('Do you confirm to Delete' + this.slcrole.desc +' ?')
      .subscribe(res => {
        if (res) {
          this.sv.delCfg(this.cfgmd).pipe().subscribe( (res) => { 
            this.toastr.success("<span class='fn-07e'>Deleted Successfully</span>",null,{ enableHtml : true });
            this. accnget(this.cacls,this.roleselect);
          });
        }
      });
