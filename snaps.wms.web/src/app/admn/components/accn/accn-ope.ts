...
export class opraccount implements OnInit, OnDestroy {
  ...
  resetpw() {
    this.ngPopups
      .confirm(`Do you reset password account ` + this.mdaccn.accncode + `?`)
      .subscribe((res) => {
        if (res) {
          this.isOn = true;
          if (this.mdaccn.tflow != `NW`) {
            this.sv
              .accnReset(this.mdaccn)
              .pipe()
              .subscribe((res) => {
  ...
  validate() {
    this.ngPopups
      .confirm(
        `Do you confirm to ` + (this.mdaccn.tflow == `NW`)
          ? `create`
          : `modify` + ` account ` + this.mdaccn.accncode + `?`
      )
      .subscribe((res) => {
        if (res) {
          this.isOn = true;
          ...
          this.sv
            .accnUpsert(this.mdaccn)
            .pipe()
            .subscribe((res) => {
              this.toastr.success(
                this.mdaccn.tflow == `NW`
                  ? `create`
                  : `modify` + ` account successful`
              );
              this.isOn = false;
  ...
  ngOnChanges() {
    ...
    // this.slwarehosue = this.lswarehouse.find(e=>e.value == this.mdaccn.site);
  }
  ngSelcompare(item, selected) {
    return item.value === selected;
  } //compare selected object with ng-select
  ngOnDestroy(): void {}
}
