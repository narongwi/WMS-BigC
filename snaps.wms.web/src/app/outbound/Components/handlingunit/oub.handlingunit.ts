...
export class oubhandlingunitComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() { this.setupJS(); /*setTimeout(this.toggle, 1000);*/ this.getmaster(); }
  setupJS() {
  ...
  confirm() {
    this.ngPopups.confirm('Do you confirm to generate handling unit  ?')
      .subscribe(res => {
        if (res) {
