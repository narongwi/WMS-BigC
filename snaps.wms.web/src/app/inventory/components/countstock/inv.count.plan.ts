...
@Component({
    ...
    styles: ['.dgtask { height:235px !important; } ',
...
export class invcounplanComponent implements OnInit, OnDestroy {
    ...
    ngSetup() {
        this.mv.getlov("COUNT", "VALIDATE").pipe().subscribe(
            (res) => {
                this.lspctval = res.sort((a, b) => Number(a.valopnfirst) - Number(b.valopnfirst));
            },
            (err) => { },
            () => { }
        );
        ...
        this.ss.storagezone().pipe().subscribe(
            (res) => { this.lszone = res; },
            (err) => { },
            () => { }
        );
    ...
    ngNew() {
        ...
        this.toastr.info("<span class='fn-07e'> Setup new plan  </span>", null, { enableHtml: true });
    }
    ngFind() {
    ...
    ngUpsert() {
        if (this.isEmpty(this.slczone.value)) {
            this.toastr.warning("<span class='fn-07e'>please select zone </span>", null, { enableHtml: true });
        } else if (!this.isbulkplan && (this.isEmpty(this.crplan.saisle) || this.isEmpty(this.crplan.eaisle))) {
            this.toastr.warning("<span class='fn-07e'>Aisle is required</span>", null, { enableHtml: true });
        } else if (!this.isbulkplan && (this.isEmpty(this.crplan.ebay) || this.isEmpty(this.crplan.ebay))) {
            this.toastr.warning("<span class='fn-07e'>Bay is required</span>", null, { enableHtml: true });
        } else if (!this.isbulkplan && (this.isEmpty(this.crplan.elevel) || this.isEmpty(this.crplan.elevel))) {
            this.toastr.warning("<span class='fn-07e'>Level is required</span>", null, { enableHtml: true });
        } else {
            this.ngPopups.confirm('Do you confirm modify plan count ?')
                .subscribe(res => {
                    if (res) {
    ...
    ngValidate() {
        if (this.crrtask.counttype == 'CT' && this.slcpct == undefined) {
            this.toastr.error("<span class='fn-07e'>Validate percentage is required !</span>", null, { enableHtml: true });
        } else {
            this.ngPopups.confirm('Do you confirm validate plan count ?')
                .subscribe(res => {
                    if (res) {
                        // stock take percentage
                        if (this.crrtask.counttype == 'CT') { this.crplan.pctvld = Number.parseInt(this.slcpct.value); }
                        this.sv.validatePlan(this.crplan).pipe().subscribe(
                            (res) => {
                                this.toastr.success("<span class='fn-07e'> Validate plan count success </span>", null, { enableHtml: true });
                                // this.ngFind(); 
                                this.ngLine();
                            ...
                            (err) => { this.toastr.error("<span class='fn-07e'> Validate plan error , " + err.message + "</span>", null, { enableHtml: true }); },
                            () => { }
                        );
                    }
                });
    ...
    /// cancel plan
    ngCancel() {
        this.ngPopups.confirm('Do you confirm cancel plan count ?')
            .subscribe(res => {
                if (res) {
                    this.sv.removePlan(this.crplan).pipe().subscribe(
                        (res) => {
                            this.toastr.success("<span class='fn-07e'> cancel plan count success </span>", null, { enableHtml: true });
                        ...
                        (err) => { this.toastr.error("<span class='fn-07e'> Cancel plan error , " + err + "</span>", null, { enableHtml: true }); },
                        () => { }
                    );
                }
            });
    ...
    // list line 
    ngLine() {
        this.sv.listLineAsync(this.crplan).pipe().subscribe(
            (res) => { this.lsline = res; },
            (err) => { this.toastr.error("<span class='fn-07e'> List plan error , " + err + "</span>", null, { enableHtml: true }); },
        );
    }
