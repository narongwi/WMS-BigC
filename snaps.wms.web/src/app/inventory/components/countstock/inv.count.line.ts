...
export class invcountlineComponent implements OnInit, OnDestroy, AfterViewInit {
	...
	ngFind() {
		this.sv
			...
			.subscribe(
				...
				(err) => {
					this.toastr.success(
						"<span class='fn-07e'>get line count error " + err + ' </span>',
						null,
						{ enableHtml: true }
					);
				}
	...
	ngSave() {
		this.ngPopups
			.confirm('Do you confirm save plan count ?')
			.subscribe((res) => {
				if (res) {
					...
					if (
						this.lsline.filter(
							(e) =>
								e.cnqtypu > 0 &&
								!this.valid(e.cnarticle) &&
								!this.valid(e.cnbarcode)
						).length > 0
					) {
						this.toastr.error(
							"<span class='fn-07e'>line count qty is required product code</span>",
							null,
							{ enableHtml: true }
						);
					} else if (
						this.lsline.filter((e) => !this.valid(e.cnqtypu)).length > 0
					) {
						this.toastr.error(
							"<span class='fn-07e'>line count qty is required </span>",
							null,
							{ enableHtml: true }
						);
					} else {
						...
						this.sv
							.upsertLineAsync(this.lsline)
							.pipe()
							.subscribe(
								() => {
									this.toastr.success(
										"<span class='fn-07e'> Save line count success </span>",
										null,
										{ enableHtml: true }
									);
								...
								() => {
									this.toastr.error(
										"<span class='fn-07e'> Save line count error , please try again.</span>",
										null,
										{ enableHtml: true }
									);
								}
	...
	public label() {
		if (!this.valid(this.editorLinemd.cnhuno)) {
			this.toastr.error("<span class='fn-07e'>Invalid HU No !</span>", null, {
				enableHtml: true,
			});
		} else {
			...
			this.sv
				.label(
					this.editorLinemd.orgcode,
					this.editorLinemd.site,
					this.editorLinemd.depot,
				...
				.subscribe(
					(response) => {
						...
						downloadLink.setAttribute(
							'download',
							'bgcwms_labelhu_' + this.editorLinemd.cnhuno + '.pdf'
						);
	...
	getdocstatement() {
		this.toastRef = this.toastr.warning("<span class='fn-07e'>Downloading ..... , Claim down wait a sec</span>",
			null,
			{
				disableTimeOut: true,
				tapToDismiss: false,
		...
		this.sv.getcountlist(this.crplan).subscribe(
			(response) => {
				...
				downloadLink.setAttribute(
					'download',
					'bgcwms_countsheet_' + this.crplan.planname + '.pdf'
				);
				document.body.appendChild(downloadLink);
				downloadLink.click();
				this.toastr.clear(this.toastRef.ToastId);
			...
			(err) => {
				this.toastr.clear(this.toastRef.ToastId);
				this.toastr.error(
					"<span class='fn-07e'> Generate Count Sheet error " +
					err.message +
					'</span>',
					null,
					{ enableHtml: true }
