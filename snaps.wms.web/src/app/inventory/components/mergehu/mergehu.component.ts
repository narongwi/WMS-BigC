...
import { merge_find, mergehu_ln, mergehu_md, merge_set, merge_md } from "../../models/inv.merge.model";
...
export class MergehuComponent implements OnInit, AfterViewInit,OnDestroy {
  ...
  public list() {
    this.sv.list(this.lsfn).subscribe((res: mergehu_md[]) => {
      this.lsmd = res;
    }, err => {
      this.toastr.error(
        "<span class='fn-07e'>" + err.message + "</span>", null,
        { enableHtml: true });
    });
  }
  public line(humd: mergehu_md, ix: number) {
    this.lssel = ix;
    this.lnsel = -1;
    this.sv.line(humd).subscribe((res: mergehu_ln[]) => {
      this.lnmd = res;
    }, err => {
      this.toastr.error(
        "<span class='fn-07e'>" + err.message + "</span>", null,
  ...
  public edithu(ix: number) {
    if (this.lsmd[ix].tflow == "IO") {
      ...
      this.toastr.warning(
        "<span class='fn-07e'> HU status is " + this.lsmd[ix].tflowdes + " </span>", null,
        { enableHtml: true });
    }
  }
  public find() {
    this.sv.find(this.mdfn).subscribe((res: mergehu_ln[]) => {
      this.mdsr = res;
    }, err => {
      this.toastr.error(
        "<span class='fn-07e'>" + err.message + "</span>", null,
  ...
  public label() {
    if (this.mdtr.hutarget == null) {
      this.toastr.error(
        "<span class='fn-07e'>Please Generate Target HU !</span>", null,
        { enableHtml: true });
    } else if (this.mdtr.tflow == "IO") {
      this.toastr.error("<span class='fn-07e'>HU is waiting confirm !</span>", null, { enableHtml: true });
    } else if (this.mdtr.tflow == "CL") {
      this.toastr.error("<span class='fn-07e'>HU is cancelled!</span>", null, { enableHtml: true });
    } else {
      ...
      this.sv.label(this.mdst.orgcode, this.mdst.site, this.mdst.depot, this.mdst.huno, "M").subscribe(response => {
        let blob: any = new Blob([response], { type: 'application/pdf' });
        const url = window.URL.createObjectURL(blob);
        let downloadLink = document.createElement('a');

        downloadLink.href = url;
        downloadLink.setAttribute('download', "bgcwms_labelhu_" + this.mdst.huno + ".pdf");
  ...
  public linklabel(ix:number) {
    ...
    if (hu.tflow == "IO") {
      ...
      this.sv.label(hu.orgcode, hu.site, hu.depot, hu.hutarget, "M").subscribe(response => {
        let blob: any = new Blob([response], { type: 'application/pdf' });
        const url = window.URL.createObjectURL(blob);
        let downloadLink = document.createElement('a');
  ...
  public setup() {
    if (this.mdst.loccode == null || this.mdst.loccode == '') {
      this.toastr.error(
        "<span class='fn-07e'>Please Enter Target Location !</span>", null,
        { enableHtml: true });
    } else {
      this.ngPopups.confirm('Do you Generate New HU ?').subscribe(res => {
        if (res) {
          this.sv.generate(this.mdst).subscribe((res: merge_md) => {
            this.mdtr = res;
            this.mdst.orgcode = this.mdtr.orgcode;
            this.mdst.site = this.mdtr.site;
            this.mdst.depot = this.mdtr.depot;
  ...
  public cancel() {
    if (this.mdtr.hutarget == null) {
      ...
      this.ngPopups.confirm('Do you cancel Merge HU ?').subscribe(res => {
        if (res) {
          this.sv.cancel(this.mdst).subscribe((res: merge_md) => {
            this.mdtr = {};
            this.mdst = {};
            this.mdtr.lines = [];
            this.list();
  ...
  public confirm() {
    if (this.mdtr.hutarget == null) {
      ...
      this.toastr.error(
        "<span class='fn-07e'>error ,merge HU item is required !</span>", null,
        { enableHtml: true });
    } else {
      this.ngPopups.confirm('Do you confirm Merge HU ?').subscribe(res => {
        if (res) {
          this.sv.merge(this.mdtr).subscribe((res: merge_md) => {
            // print hu label
            this.label();
            this.mdtr = {};
            this.mdst = {};
            ...
            this.list();
            this.toastr.success(
              "<span class='fn-07e'>Confirm Successfully!</span>", null,
              { enableHtml: true });
          }, err => {
            this.toastr.error(
              "<span class='fn-07e'>" + err.message + "</span>", null,
  ...
  public select(index: number) {
    if (this.mdtr.hutarget == null) {
      ...
      if (this.mdtr.lines.filter(e => e.loccode == sln.loccode && e.huno == sln.huno).length > 0) {
        this.toastr.warning(
          "<span class='fn-07e'>" + sln.huno + " already exists in list , </span>", null,
          { enableHtml: true });
      } else {
