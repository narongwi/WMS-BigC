...
declare var $: any;
@Component({
  selector: 'app-extstorage',
  templateUrl: 'extstorage.component.html'

})
export class extstorageComponent implements OnInit {
  ...
  meUploadEvent(event){
    if (event.length > 0) {
      ...
      if (!this.fileformat.includes(file.type)) {
        ...
        this.sv.uploadLocup(formData,this.ngDecFiletype(file.type)).subscribe(            
