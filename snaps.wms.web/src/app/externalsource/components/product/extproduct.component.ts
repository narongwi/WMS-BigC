...
declare var $: any;
@Component({
  selector: 'app-extproduct',
  templateUrl: 'extproduct.component.html'

})
export class extproductComponent implements OnInit {
  ...
  meUploadEvent(event){
    if (event.length > 0) {
      ...
      if (!this.fileformat.includes(file.type)) {
        ...
        this.sv.uploadProduct(formData,this.ngDecFiletype(file.type)).subscribe(            
