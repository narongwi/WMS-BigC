...
import { extordinbsource } from './extordinb.source';

declare var $: any;
@Component({
  selector: 'app-extordinb',
  templateUrl: 'extordinb.component.html',
...
export class extordinbComponent implements OnInit {
  @ViewChild('line') line:extordinbsource;
  //List file update
  public files: any = [];
  //PageNavigate
  ...
  meUploadEvent(event){
    if (event.length > 0) {
      ...
      if (!this.fileformat.includes(file.type)) {
        ...
        this.sv.uploadInbound(formData,this.ngDecFiletype(file.type)).subscribe(            
