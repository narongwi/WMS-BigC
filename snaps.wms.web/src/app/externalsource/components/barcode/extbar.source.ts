...
@Component({
  selector: 'appext-barsource',
  templateUrl: 'extbar.source.html',
  styles: ['.dgexsource { height:200px !important; } ','.dgexline { height:calc(100vh - 480px) !important; } '] ,
  providers: [
    {provide: NgbDateAdapter,         useClass: CustomAdapter},
    {provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter}]    
})
export class extbarsource implements OnInit, OnDestroy {
  @Input() item: lov;
  ...
  public lsbarcode:exsBarcode[] = new Array();
  ...
  constructor(private ss:shareService, private sv:externalSourcelService) { this.ss.ngSetup(); this.ngSetup(); this.pm.imptype = "barcode";  }
  ...
  meFnd() { this.sv.findBarcode(this.pm).subscribe((res) => {this.lssource = res; if(this.lssource.length > 0) { this.meLine(this.lssource[0]); }});}
  meLine(o:exsFile) { this.sv.getBarcodeLines(o).subscribe((res) => { this.lsbarcode = res;}); }
