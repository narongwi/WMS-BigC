...
import { exsPrep } from '../../models/snaps.wms.externalsource.outbound';
@Component({
  selector: 'appext-prepssource',
  templateUrl: 'extpreps.source.html',
  styles: ['.dgexsource { height:200px !important; } ','.dgexline { height:calc(100vh - 480px) !important; } '] ,
  providers: [
    {provide: NgbDateAdapter,         useClass: CustomAdapter},
    {provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter}]    
})
export class extprepssource implements OnInit {
  @Input() item: lov;
  ...
  public lsline:exsPrep[] = new Array();
  public lssource:exsFile[] = new Array();
  //public lspreps:exspreps[] = new Array();
  ...
  constructor(private ss:shareService, private sv:externalSourcelService) { this.ss.ngSetup(); this.ngSetup(); this.pm.imptype = "prep"; }
  ...
  meFnd() { this.sv.findPrep(this.pm).subscribe((res) => {this.lssource = res; if(this.lssource.length > 0) { this.meLine(this.lssource[0]); }});}
  meLine(o:exsFile) { this.sv.getPrepLines(o).subscribe((res) => { this.lsline = res;}); }
