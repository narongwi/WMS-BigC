...
declare var $: any;
@Component({
  selector: 'app-extpreps',
  templateUrl: 'extpreps.component.html'

})
export class extprepsComponent implements OnInit {
  ...
  meUploadEvent(event){
    if (event.length > 0) {
      ...
      if (!this.fileformat.includes(file.type)) {
        ...
        this.sv.uploadPrep(formData,this.ngDecFiletype(file.type)).subscribe(            
