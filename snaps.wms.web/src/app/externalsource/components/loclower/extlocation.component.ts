...
declare var $: any;
@Component({
  selector: 'app-extlocation',
  templateUrl: 'extlocation.component.html'

})
export class extlocationComponent implements OnInit {
  ...
  meUploadEvent(event){
    if (event.length > 0) {
      ...
      if (!this.fileformat.includes(file.type)) {
        ...
        this.sv.uploadLocdw(formData,this.ngDecFiletype(file.type)).subscribe(            
