...
import { exsLocdw } from '../../models/snaps.wms.externalsource.outbound';
@Component({
  selector: 'appext-locationsource',
  templateUrl: 'extlocation.source.html',
  styles: ['.dgexsource { height:200px !important; } ','.dgexline { height:calc(100vh - 480px) !important; } '] ,
  providers: [
    {provide: NgbDateAdapter,         useClass: CustomAdapter},
    {provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter}]    
})
export class extlocationsource implements OnInit {
  @Input() item: lov;
  ...
  public lsline:exsLocdw[] = new Array();
  public lssource:exsFile[] = new Array();
  //public lslocation:exslocation[] = new Array();
  ...
  constructor(private ss:shareService, private sv:externalSourcelService) { this.ss.ngSetup(); this.ngSetup(); this.pm.imptype = "locdw"; }
  ...
  meFnd() { this.sv.findLocdw(this.pm).subscribe((res) => {this.lssource = res; if(this.lssource.length > 0) { this.meLine(this.lssource[0]); }});}
  meLine(o:exsFile) { this.sv.getLocdwLines(o).subscribe((res) => { this.lsline = res;}); }
