...
import { extordoubsource } from './extordoub.source';

declare var $: any;
@Component({
  selector: 'app-extordoub',
  templateUrl: 'extordoub.component.html',
  styles: ['.dgexsource { height:200px !important; } ','.dgstockline { height:calc(100vh - 470px) !important; } ','.dgcorrect { height:calc(100vh - 195px) !important; } '] ,
  providers: [
    {provide: NgbDateAdapter,         useClass: CustomAdapter},
    {provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter}]    
})
export class extordoubComponent implements OnInit {
  @ViewChild('line') line:extordoubsource;
  //List file update
  public files: any = [];
  //PageNavigate
  ...
  meUploadEvent(event){
    if (event.length > 0) {
      ...
      if (!this.fileformat.includes(file.type)) {
        ...
        this.sv.uploadOutbound(formData,this.ngDecFiletype(file.type)).subscribe(            
