...
@Component({
selector: 'appext-ordoulcomponent',
templateUrl: 'extordoul.component.html',
styles: ['.dgexsource { height:200px !important; } ','.dgexline { height:calc(100vh - 480px) !important; } '] ,
providers: [
    {provide: NgbDateAdapter,         useClass: CustomAdapter},
    {provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter}]    
})
export class extordoulcomponent implements OnInit {
  //List file update
  public files: any = [];
  //PageNavigate
  ...
  meUploadEvent(event){
    if (event.length > 0) {
      ...
      if (!this.fileformat.includes(file.type)) {
        ...
        this.sv.uploadOutbouln(formData,this.ngDecFiletype(file.type)).subscribe(            
