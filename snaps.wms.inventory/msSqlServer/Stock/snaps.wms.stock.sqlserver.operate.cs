        ...
        public List<SqlCommand> stockIncommand(ref List<stock_mvin> o,ref SqlConnection ocn,ref SqlTransaction otx,ref Decimal outstockid) {
            ...
            try {
                ...
                foreach(stock_mvin ln in o) {
                    ...
                    // inrefno
                    if(ln.opssku <= 0) { throw new Exception("Quantity must more than 0 "); } else if(rsl == "HUweight") { throw new Exception(string.Format("Weight quantity on HU {0} is over capacity",ln.opshuno)); }                   //Check summary weight on HU is over
